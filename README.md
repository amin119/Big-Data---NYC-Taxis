...
| Phase | Name | Status |
|-------|------|--------|
| 0 | Infrastructure — Docker Compose (7 services) | ✅ Done |
...
| 4 | Real-time Dashboards — Grafana live heatmap + metrics | ✅ Done |
...
| `zone-api` | custom (docker/Dockerfile.zone-api) | **5001** | Live zone JSON API for Grafana Geomap |
...
> **`zone-api`** is a lightweight Python HTTP server (built from `docker/Dockerfile.zone-api`) that reads `zone_map_stats` from Cassandra and exposes `GET /zones` as a JSON array. Grafana's Infinity datasource polls it every 30 s to refresh the live heatmap. It waits for Cassandra's healthcheck before starting.
...
├── docker/
│   └── Dockerfile.zone-api         ← ✅ Phase 4: Zone API image
...
│   ├── load_zone_centroids.py      ← ✅ Phase 4: Seed zone lat/lon into Cassandra
│   └── zone_api.py                 ← ✅ Phase 4: JSON API for Grafana Geomap
├── spark/
│   ├── streaming_job.py            ← ✅ Phase 3: Spark Structured Streaming
│   └── batch_job.py                ← ✅ Phase 5: Spark batch + MLlib
├── cassandra/
│   └── init.cql                    ← ✅ Phase 3+4: Keyspace + 4 tables
├── airflow/
│   └── dags/
│       └── taxi_batch_dag.py       ← ✅ Phase 5: Airflow DAG
├── zone_centroids.json             ← Generated by load_zone_centroids.py (git-ignored)
└── README.md
```
...
### 7 — Phase 4: Live Grafana Heatmap

#### 7a — Seed zone centroids (run once)

Downloads the TLC taxi zone shapefile, converts polygon centroids from NY State Plane (EPSG:2263) to WGS84, and writes 263 rows into `zone_map_stats`:
...
Expected output:
```
Computed centroids for 263 zones
Connecting to Cassandra localhost:9042 ... OK
Inserted 263 rows into zone_map_stats (snapshot='current')
Saved 263 centroids → zone_centroids.json
```

This also writes `zone_centroids.json` to the project root — the streaming job loads it at startup to avoid a cassandra-driver dependency inside PySpark.

#### 7b — Start the Zone API

The Zone API is part of the Docker stack. After `docker compose up -d`, the `zone-api` container starts automatically once Cassandra is healthy. Verify it is running:

```bash
curl http://localhost:5001/zones | python -m json.tool | head -20
# Should print 263 JSON objects with zone_id, lat, lon, trip_count, …
```

#### 7c — Configure Grafana Live Dashboard

Open <http://localhost:3001> (admin / admin).

#### Step 1 — Add Infinity datasource

1. Left sidebar → **Connections** → **Data sources** → **Add new data source**
2. Search **"Infinity"** → select it → **Save & Test**

#### Step 2 — Create the Live Heatmap panel

1. **Dashboards** → **New** → **New dashboard** → **Add visualization**
2. Select **Infinity** as the datasource
3. In the query editor set:
   - Type: `JSON` | Source: `URL` | Method: `GET`
   - URL: `http://zone-api:5001/zones`
   - Format: `Table`
4. Click **Run queries** — 263 rows should appear
5. Switch the visualization type to **Geomap** (top-right panel picker)
6. In panel options → **Map layers** → Layer 1:
   - Layer type: `Heatmap`
   - Location mode: `Coords`
   - Latitude field: `lat` | Longitude field: `lon`
   - Color by: `trip_count`
7. Set dashboard auto-refresh to **30s**
8. Save dashboard as **"NYC Taxi — Live"**

The heatmap updates automatically as Spark Streaming writes new trip counts to `zone_map_stats` every 30 seconds.
...
Full schema in [cassandra/init.cql](cassandra/init.cql). Apply manually once after starting the stack:
...
|-------|-----------|---------|---------|
| `trip_stats_by_window` | Spark Streaming (30 s) | Grafana | 5-min window counts, fares, predicted demand |
| `weather_snapshots` | Spark Streaming (10 s) | Grafana | Current NYC conditions |
| `daily_stats` | Spark Batch (Phase 5) | Grafana | Daily aggregates per zone |
| `zone_map_stats` | `load_zone_centroids.py` (seed) + Spark Streaming (30 s) | zone-api → Grafana Geomap | Live zone lat/lon + trip counts for heatmap |
...
-- Live heatmap: one row per zone, upserted every 30 s by streaming job
-- snapshot='current' is the single partition — returns all 263 zones in one query
CREATE TABLE zone_map_stats (
    snapshot text, zone_id int,
    lat double, lon double,
    zone_name text, borough text,
    trip_count bigint, predicted_demand int,
    weather_label text, updated_at timestamp,
...
|-----------|-------------|--------|
| Phase 0 | `docker compose ps` — all 7 services running/healthy | ✅ |
...
| Phase 4 | `curl localhost:5001/zones` returns 263 zones; Grafana Geomap heatmap live at <http://localhost:3001> | ✅ |
